...
FOR EACH ROW
BEGIN
	-- Lôi logic: Đang lầy ngày cũ ra so sánh với hiện tại thay vì kiểm tra ngày mới
	IF OLD.appointment_date < NOW( ) THEN
	SIGNAL SQLSTATE '45000' 
    SET MESSAGE_TEXT = 'Lỗi: Không thể đặt lịch khám vào thởi điểm trong quá khứ!';
...
-- Viết câu lệnh UPDATE để tái hiện lỗi dời lịch.
UPDATE Appointments 
SET appointment_date = '2026-05-01 09:00:00'
WHERE appointment_id = 104;
-- Giải thích ngắn gọn (2–3 dòng): Sự khác biệt cốt lõi giữa OLD.appointment_date và NEW.appointment_date trong thao tác này là gì? Vì sao đoạn mã trên lại không chặn được lỗi?

-- OLD.appointment_date là giá trị ngày hẹn trước khi UPDATE, còn NEW.appointment_date là giá trị mới sau khi UPDATE.
-- Trigger cũ kiểm tra OLD.appointment_date < NOW(), tức là ngày cũ, nên không phát hiện được nếu người dùng đổi ngày mới thành quá khứ. Vì vậy lỗi không bị chặn.

-- Phần B: Sửa chữa mã nguồn

-- Viết mã lệnh SQL để xóa (DROP) trigger PreventPastAppointments cũ.
DROP TRIGGER PreventPastAppointments;
-- Viết lại mã lệnh tạo mới (CREATE) trigger PreventPastAppointments đúng logic.
DELIMITER //
